...
// Material Icons (remain the same)
const PlayIcon = () => (
...
export default function AudioPlayer() {
  ...
  // State to manage rendering for exit animations
  ...
  // Effect for player visibility and animation
  useEffect(() => {
    if (isPlayerVisible) {
      setShouldRenderPlayer(true); // Render immediately to start animation
    } else {
      // Delay unrendering to allow CSS exit animation to complete
      const timer = setTimeout(() => {
        setShouldRenderPlayer(false);
      }, 300); // Should match CSS animation/transition duration for exit
  ...
  useEffect(() => {
    ...
    if (audio.src !== newSrcAbsolute) {
      audio.src = currentSong.src;
    }
    if (isPlaying) audio.play().catch(e => { console.error("Play error:", e); setIsPlaying(false); });
    else audio.pause();
  ...
  useEffect(() => {
    ...
    if (audio.readyState >= 1 && audio.src === new URL(songs[currentSongIndex]?.src, window.location.origin).href) loadMeta();
    ...
    <>
      <style>{`
        ...
        :root {
          ...
          --md-sys-color-tertiary-container: #FFD8E4; /* For FAB hover example */
        ...
        .fab-toggle-button {
          ...
          border: none;
          box-shadow: var(--md-sys-elevation-level2); /* Standard elevation */
          cursor: pointer;
          display: flex;
          align-items: center;
        ...
        .fab-toggle-button.fab-hidden {
          opacity: 0;
          transform: scale(0.8) translateY(10px);
          pointer-events: none;
        }
        .fab-toggle-button:hover:not(.fab-hidden) {
          transform: scale(1.05) translateY(-2px); /* Slight lift and scale */
          box-shadow: var(--md-sys-elevation-level3); /* Increased elevation on hover */
          background-color: var(--md-sys-color-tertiary-container);
        }
        .fab-toggle-button:active:not(.fab-hidden) {
        ...
        .player-header .icon-button:hover {
          background-color: color-mix(in srgb, var(--md-sys-color-on-surface-variant) 8%, transparent);
        }
        .player-header .icon-button svg { width: 24px; height: 24px; }

        .song-details { text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; }
        .album-art { width: 120px; height: 120px; object-fit: cover; border-radius: var(--md-sys-shape-corner-medium); box-shadow: var(--md-sys-elevation-level1); }
        .song-details h3 { margin: 0; font: var(--md-sys-typescale-title-large-font-weight) var(--md-sys-typescale-title-large-font-size)/var(--md-sys-typescale-title-large-line-height) var(--md-sys-typescale-title-large-font-family); color: var(--md-sys-color-on-surface); }
        .song-details p { margin: 0; font: var(--md-sys-typescale-body-medium-font-weight) var(--md-sys-typescale-body-medium-font-size)/var(--md-sys-typescale-body-medium-line-height) var(--md-sys-typescale-body-medium-font-family); color: var(--md-sys-color-on-surface-variant); }
        ...
        .progress-bar-container:hover .progress-bar { background-color: color-mix(in srgb, var(--md-sys-color-primary) 80%, var(--md-sys-color-on-primary)); } /* Slightly lighter on hover */


        .time-display { display: flex; justify-content: space-between; font: var(--md-sys-typescale-label-medium-font-weight) var(--md-sys-typescale-label-medium-font-size)/var(--md-sys-typescale-label-medium-line-height) var(--md-sys-typescale-label-medium-font-family); color: var(--md-sys-color-on-surface-variant); padding: 0 4px; }

        .controls { display: flex; justify-content: center; align-items: center; gap: 12px; }
        .control-button {
          background-color: transparent; border: none; color: var(--md-sys-color-on-surface-variant);
        ...
        .control-button.play-pause-button:hover:not(:disabled) {
          background-color: var(--md-sys-color-primary); color: var(--md-sys-color-on-primary);
          transform: scale(1.05); box-shadow: var(--md-sys-elevation-level1);
        }
        .control-button:active:not(:disabled) { transform: scale(0.95); } /* General active scale */
        .control-button.play-pause-button:active:not(:disabled) { transform: scale(0.92); }

        /* CSS Ripple Effect */
        .control-button::after, .track-item::after, .fab-toggle-button::after, .player-header .icon-button::after {
        ...
        .control-button.play-pause-button::after { background: var(--md-sys-color-on-primary); } /* Specific for play/pause */
        ...
        /* Track item ripple will use its text color, or you can specify if needed */

        .control-button:active::after, .track-item:active::after, .fab-toggle-button:active::after, .player-header .icon-button:active::after {
          transition: transform 0.4s cubic-bezier(0, 0, 0.2, 1), opacity 0.5s cubic-bezier(0, 0, 0.2, 1);
          transform: scale(35) translate(-50%, -50%); /* Adjust scale for coverage */
          opacity: 0.12; /* Ripple opacity */
        }


        .track-list { max-height: 150px; overflow-y: auto; background-color: var(--md-sys-color-surface); border-radius: var(--md-sys-shape-corner-small); }
        .track-item {
        ...
        .track-item:hover:not(.active) {
          ...
          transform: translateY(-1px); box-shadow: var(--md-sys-elevation-level1);
        }
        ...
        .track-title { font: var(--md-sys-typescale-body-large-font-weight) var(--md-sys-typescale-body-large-font-size)/var(--md-sys-typescale-body-large-line-height) var(--md-sys-typescale-body-large-font-family); color: var(--md-sys-color-on-surface); }
        .track-artist { font: var(--md-sys-typescale-body-medium-font-weight) var(--md-sys-typescale-body-medium-font-size)/var(--md-sys-typescale-body-medium-line-height) var(--md-sys-typescale-body-medium-font-family); color: var(--md-sys-color-on-surface-variant); }
      `}</style>
